...
private:
	...
	void TeleopInit()
	{
		//Example Code- Won't Do Much
		std::shared_ptr<NetworkTable> test;

		test = NetworkTable::GetTable("test");

		test->PutBoolean("bool", false);
		double num = test->GetNumber("num", 0);

		test->PutNumber("num", ++num);
		test->PutString("str", "Hello, world!");
	}
